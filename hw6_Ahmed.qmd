---
title: "Assignment 6"
author: "Inaya Ahmed"
format: html
editor: visual
---

## DATA-613 Assignment 6

### Part I: Answer the following question by running SQL using RStudio. 

```{r}
# Loading libraries
library(DBI)
library(duckdb)
library(nycflights13)

# Downloading duckdb file
download.file(
  "https://data-science-master.github.io/lectures/data/flights.duckdb",
  destfile = "flights.duckdb",
  mode = "wb")

# Connecting to it
conn <- dbConnect(duckdb(), dbdir = "flights.duckdb", read_only = TRUE)

# Verifying connection
dbListTables(conn)
```

#### a. List the first 10 rows of the flights table.

```{r}
dbGetQuery(conn, "
  SELECT *
  FROM flights
  LIMIT 10
")
```

#### **b.**

Find the **number of flights**

i.  in the dataset

ii. for each carrier

```{r}
# part i
dbGetQuery(conn, "
  SELECT COUNT(*) AS total_flights
  FROM flights
")

# the total number of flights in the dataset is 336,776.
```

```{r}
# part ii
dbGetQuery(conn, "
  SELECT carrier, COUNT(*) AS num_flights
  FROM flights
  GROUP BY carrier
  ORDER BY num_flights DESC
")

# the total number of flights for each carrier can be seen in the table below:
```

#### **c.**

Find the **average departure delay** (`dep_delay`)

```{r}
dbGetQuery(conn, "
           SELECT AVG(dep_delay) AS avg_dep_delay
           FROM flights
           ")

# The average departure delay is 12.64 minutes (rounded to the nearest hundredth).
```

#### **d.**

Find the **top 5 destinations** (`dest`) With the most flights.

-   Your output should include:

    -   destination

    -   number of flights Sort from the **largest to smallest**.

```{r}
dbGetQuery(conn, "
           SELECT dest, COUNT(*) AS num_flights
           FROM flights
           GROUP BY dest
           ORDER BY num_flights DESC
           LIMIT 5
           ")

# the top 5 destinations with their corresponding number of flights can be seen below, sorted from largest to smallest:
```

#### **e.**

Create a table showing:

-   `carrier`

-   average arrival delay (`arr_delay`)

Sort the results from **largest to smallest delay**

```{r}
dbGetQuery(conn, "
  SELECT carrier, AVG(arr_delay) AS avg_arr_delay
  FROM flights
  GROUP BY carrier
  ORDER BY avg_arr_delay DESC
")

# please see the below table illustrating average departure delay by carrier, sorted from largest to smallest:
```

> You must close the connection when you are done. Use `dbDisconnect()`

```{r}
dbDisconnect(conn)
```

### **Part II: Answer the following question by running SQL using DBeaver with DuckDB.**

> Please go over the note **dbeaver_sql.html** to learn

#### **a. List the first 10 rows of the `flights` table:**

*The first 10 rows of the flights table are seen in the below screenshots. I had to take two in order to display all of the columns.*

![](images/clipboard-1929981447.png)

![](images/clipboard-130832666.png)

#### **b.**

Count the Number of Flights for Each Carrier

Find the **number of flights** for each carrier.

-   Your output should include:

    -   carrier

    -   number of flights Sort from the **largest to smallest**.

*The total of flights per carrier can be seen below, sorted from largest to smallest:*

![](images/clipboard-344444069.png)

#### **c.**

Find the **average departure delayby month**

-   Your output should include:

    -   month

    -   average departure delay

*Here you can see the average departure delay by month, sorted chronologically from 1-12:*

![](images/clipboard-1793852608.png)

#### **d.**

Find the **top 10 routes** (`dest`) by:

-   `origin`

-   `dest`

ordered by **number of flights**.

*See here the top 10 routes, sorted from most to least total flights:*

![](images/clipboard-181261075.png)

**e.**

Export the result of part b query to a **CSV file** using the **Export Data** feature in DBeaver.

Submit the exported CSV file along with your homework.

*The .csv file titled "hw6_Ahmed_part 2b data.csv" displays my output for this question.*

**Exporting Data to CSV in DBeaver**

-   **Quick Steps:**

1.  **Right-click** Table or Result Grid -\> **Export Data**.

2.  Select **CSV** -\> Click **Next**.

3.  Confirm **Settings** (Delimiters, Encoding) -\> Click **Next**.

4.  Choose **Output Directory** -\> Click **Finish**.

## **Remember to Close the connection When You Are Done**

-   **How to Disconnect and Reconnect a Database in DBeaver**

1.  On the **top menu**, click **Database**.

2.  From the drop-down menu, select **Disconnect**.

3.  You should see a message on the **bottom right corner**:

    ```         
    Not connected to database
    ```

4.  To reconnect, go to **Database → Connect** and select your connection.

> This is the easiest way to manage connections without closing DBeaver or losing work.

*See below proof of disconnection from database:*

![](images/clipboard-3324008482.png)

## **Submission**

-   Save your Quarto file after completing each problem, render it, then stage and commit the changes.

-   Include well-commented code and clear explanations.

-   Make appropriate **commits**.

-   Submit the rendered file to **Canvas** and push it to your **GitHub repository**.

**Good luck!**
